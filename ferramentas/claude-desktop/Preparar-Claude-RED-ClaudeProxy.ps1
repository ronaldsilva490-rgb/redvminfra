...
$proxyBaseUrl = if ($env:RED_PROXY_BASE_URL) { $env:RED_PROXY_BASE_URL.TrimEnd("/") } else { "https://redsystems.ddns.net/redclaudeproxy" }
$proxyToken = if ($env:RED_PROXY_TOKEN) { $env:RED_PROXY_TOKEN } else { "red" }
$profileName = "RED Claude Proxy"
...
function Get-ProxyModels {
  try {
    ...
    Write-Warn "Catalogo do redclaudeproxy veio pequeno; usando lista local."
  } catch {
...
Write-Host "  Claude Desktop - RED Claude Proxy" -ForegroundColor Red
Write-Host "===============================================" -ForegroundColor DarkRed
Write-Host ""
