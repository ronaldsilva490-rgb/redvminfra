...
$orgId = "f24a9483-2a42-4d7b-8fb6-fd304cd0ef4a"
$proxyBaseUrl = if ($env:RED_NIM_CLAUDE_BASE_URL) { $env:RED_NIM_CLAUDE_BASE_URL.TrimEnd("/") } else { "https://redsystems.ddns.net:5050" }
$proxyToken = if ($env:RED_NIM_CLAUDE_TOKEN) { $env:RED_NIM_CLAUDE_TOKEN } else { "red" }
$profileName = "RED NIM Claude 5050"
...
function Backup-File([string]$path) {
  ...
  $backupDir = Join-Path $dir "rednimclaude-config-backups"
...
function Get-ProxyModels {
  try {
    ...
    Write-Warn "Catalogo do RED NIM Claude veio vazio; usando fallback local."
  } catch {
...
Write-Host "  Claude Desktop - RED NIM Claude :5050" -ForegroundColor Red
Write-Host "===========================================" -ForegroundColor DarkRed
Write-Host ""
