...
$proxyBaseUrl = if ($env:RED_LIGHTNING_CLAUDE_BASE_URL) { $env:RED_LIGHTNING_CLAUDE_BASE_URL.TrimEnd("/") } else { "https://redsystems.ddns.net:5051" }
$proxyToken = if ($env:RED_LIGHTNING_CLAUDE_TOKEN) { $env:RED_LIGHTNING_CLAUDE_TOKEN } else { "red" }
$profileName = "RED Lightning Claude 5051"
...
function Backup-File([string]$path) {
  ...
  $backupDir = Join-Path $dir "redlightningclaude-config-backups"
...
function Get-ProxyModels {
  try {
    ...
    Write-Warn "Catalogo do RED Lightning Claude veio vazio; usando fallback local."
  } catch {
...
Write-Host "  Claude Desktop - RED Lightning Claude :5051" -ForegroundColor Red
Write-Host "=================================================" -ForegroundColor DarkRed
Write-Host ""
