...
if (!empty($_SERVER['HTTP_X_FORWARDED_PREFIX'])) {
	...
	if ($forwardedPrefix !== '/' && strpos($_SERVER['SCRIPT_NAME'], $forwardedPrefix . '/') !== 0) {
		$_SERVER['SCRIPT_NAME'] = $forwardedPrefix . $_SERVER['SCRIPT_NAME'];
	}
	if (!empty($_SERVER['PHP_SELF']) && $forwardedPrefix !== '/' && strpos($_SERVER['PHP_SELF'], $forwardedPrefix . '/') !== 0) {
		$_SERVER['PHP_SELF'] = $forwardedPrefix . $_SERVER['PHP_SELF'];
	}
}
...
define('TEMPLATE_DIR', 'templates/red/');
