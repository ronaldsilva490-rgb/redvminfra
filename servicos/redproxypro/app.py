...
VERCEL_BASE_URL = os.getenv("REDPROXYPRO_VERCEL_BASE_URL", "https://ai-gateway.vercel.sh/v1").rstrip("/")
HOST = os.getenv("REDPROXYPRO_HOST", "127.0.0.1")
PORT = env_int("REDPROXYPRO_PORT", 8095)
...
DATA_DIR = Path(os.getenv("REDPROXYPRO_DATA_DIR", "/var/lib/redproxypro"))
USAGE_FILE = Path(os.getenv("REDPROXYPRO_USAGE_FILE", str(DATA_DIR / "usage.json")))
CLAUDE_UNKNOWN_MODEL_FALLBACK = env_bool("REDPROXYPRO_CLAUDE_UNKNOWN_MODEL_FALLBACK", True)
CLAUDE_STICKY_TTL_SECONDS = env_int("REDPROXYPRO_CLAUDE_STICKY_TTL_SECONDS", 6 * 60 * 60)
...
def build_claude_alias_by_id() -> dict[str, dict[str, Any]]:
    aliases: dict[str, dict[str, Any]] = {}
    for item in CLAUDE_MODEL_ALIASES:
        for model_id in [item["id"], *item.get("aliases", [])]:
            aliases[str(model_id).lower()] = item
    return aliases


CLAUDE_ALIAS_BY_ID = build_claude_alias_by_id()
CLAUDE_ALIAS_BY_TARGET = {item["target"].lower(): item for item in CLAUDE_MODEL_ALIASES}
...
def default_claude_alias() -> dict[str, Any]:
    for item in CLAUDE_MODEL_ALIASES:
        if item.get("default"):
            return item
    return sorted(CLAUDE_MODEL_ALIASES, key=lambda item: str(item["target"]).lower())[0]
...
def last_claude_alias_for_request() -> dict[str, Any] | None:
    ...
    with CLAUDE_CLIENT_MODEL_LOCK:
        ...
        if timestamp < now - max(60, CLAUDE_STICKY_TTL_SECONDS):
            ...
            return None
    return CLAUDE_ALIAS_BY_ID.get(alias_id.lower())
...
def extract_usage(payload: Any) -> dict[str, Any]:
    ...
    total_cost = first_number(
        usage,
        payload,
        keys=("cost", "total_cost", "estimated_cost", "redproxypro_cost", "vercel_cost"),
    )
...
def load_keys() -> list[KeyState]:
    ...
    keys_file = os.getenv("REDPROXYPRO_KEYS_FILE", "").strip()
    if keys_file:
        try:
            ...
            pass
    raw_entries.extend(split_values(os.getenv("REDPROXYPRO_KEYS", "")))
    keys: list[KeyState] = []
    seen_names: set[str] = set()
    seen_keys: set[str] = set()
    for index, entry in enumerate(raw_entries, start=1):
        parsed = parse_key_entry(entry, index)
        if not parsed or not parsed.key or not parsed.key.startswith("vck_"):
            continue
...
def cooldown_for_status(status: int) -> int:
    ...
    if status == 429:
        return env_int("REDPROXYPRO_COOLDOWN_429", 180)
    if status >= 500:
        return env_int("REDPROXYPRO_COOLDOWN_5XX", 60)
...
def resolve_claude_model(
    ...
    *,
    ...
    alias = CLAUDE_ALIAS_BY_ID.get(model_id.lower()) or CLAUDE_ALIAS_BY_TARGET.get(model_id.lower())
    ...
    if allow_fallback and CLAUDE_UNKNOWN_MODEL_FALLBACK:
        ...
        if log_key not in UNKNOWN_CLAUDE_MODELS_LOGGED:
            ...
            print(
                f"[redproxypro] Claude gateway unknown model {model_id!r}; using fallback {fallback['id']!r}",
                flush=True,
            )
        return fallback
    return None
...
def authorize() -> Response | None:
    ...
    if inbound_token() in INBOUND_TOKENS:
        return None
    return jsonify({"error": {"message": "redproxypro authorization required", "type": "authentication_error"}}), 401
...
def add_cors_headers(response: Response) -> Response:
    ...
    response.headers["Access-Control-Expose-Headers"] = "Content-Type, X-Request-Id, X-RedProxyPro-Key, X-RedProxyPro-Attempts"
...
def response_headers(resp: requests.Response, key_name: str, attempts: list[dict[str, Any]]) -> dict[str, str]:
    ...
    for name, value in resp.headers.items():
        if name.lower() not in HOP_BY_HOP_HEADERS:
            headers[name] = value
    headers["X-RedProxyPro-Key"] = key_name
    headers["X-RedProxyPro-Attempts"] = str(len(attempts))
...
def proxy_v1(path: str) -> Response:
    ...
    if not key_pool.keys:
        return jsonify({"error": {"message": "REDPROXYPRO_KEYS is empty", "type": "configuration_error"}}), 503

    upstream_url = f"{VERCEL_BASE_URL}/{path.lstrip('/')}"
...
def upstream_chat_completion(payload: dict[str, Any], *, endpoint: str, stream: bool) -> tuple[requests.Response | None, KeyState | None, list[dict[str, Any]], Response | None]:
    upstream_url = f"{VERCEL_BASE_URL}/chat/completions"
...
def claude_models():
    ...
    if auth_error is not None:
        return auth_error
    models = sorted(CLAUDE_MODEL_ALIASES, key=lambda item: str(item["id"]).lower())
...
def anthropic_count_tokens():
    ...
    if alias is None:
        return jsonify({"error": {"message": "modelo indisponivel no Claude gateway do RED Proxy Pro", "type": "invalid_request_error"}}), 400
...
def anthropic_messages():
    ...
    if alias is None:
        return jsonify(
            {
                "error": {
                    "message": "modelo indisponivel no Claude gateway do RED Proxy Pro",
                    "type": "invalid_request_error",
                },
                "available_models": [item["id"] for item in CLAUDE_MODEL_ALIASES],
            }
        ), 400
