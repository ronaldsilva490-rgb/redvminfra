...
function buildPortableLauncherBat() {
  ...
  const powershellScript = [
    ...
    "function B($label,$current,$total){if($total -lt 1){$total=1};$ratio=[Math]::Min([Math]::Max(($current/[double]$total),0),1);$width=28;$fill=[int][Math]::Round($ratio*$width);if($fill -gt $width){$fill=$width};$bar=('='*$fill).PadRight($width,' ');$pct=[int][Math]::Round($ratio*100);Write-Host -NoNewline ('`r'+$label+' ['+$bar+'] '+$pct.ToString().PadLeft(3)+'%');if($current -ge $total){Write-Host ''}}",
    "function DownloadFile($url,$dest){$req=[System.Net.HttpWebRequest]::Create($url);$req.AutomaticDecompression=[System.Net.DecompressionMethods]::GZip -bor [System.Net.DecompressionMethods]::Deflate;$resp=$req.GetResponse();try{$total=[int64]$resp.ContentLength;$stream=$resp.GetResponseStream();$out=[System.IO.File]::Create($dest);try{$buffer=New-Object byte[] 65536;$readTotal=[int64]0;while(($read=$stream.Read($buffer,0,$buffer.Length)) -gt 0){$out.Write($buffer,0,$read);$readTotal+=$read;if($total -gt 0){B 'Instalando dependencias' $readTotal $total}};if($total -le 0){B 'Instalando dependencias' 1 1}}finally{$out.Dispose();$stream.Dispose()}}finally{$resp.Dispose()}}",
    "function ExtractZipWithProgress($zip,$dest){Add-Type -AssemblyName System.IO.Compression.FileSystem;$archive=[System.IO.Compression.ZipFile]::OpenRead($zip);try{$entries=@($archive.Entries);$total=[Math]::Max($entries.Count,1);$index=0;foreach($entry in $entries){$target=Join-Path $dest $entry.FullName;if([string]::IsNullOrWhiteSpace($entry.Name)){New-Item -ItemType Directory -Path $target -Force|Out-Null}else{$dir=Split-Path $target -Parent;if($dir){New-Item -ItemType Directory -Path $dir -Force|Out-Null};[System.IO.Compression.ZipFileExtensions]::ExtractToFile($entry,$target,$true)};$index++;B 'Extraindo dependencias ' $index $total}}finally{$archive.Dispose()}}",
    "try{",
    ...
    "$e=Get-ChildItem -LiteralPath $t -Filter 'SafeExamBrowser.exe' -Recurse -File -ErrorAction SilentlyContinue|Select-Object -First 1 -ExpandProperty FullName",
