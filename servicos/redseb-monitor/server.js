...
function buildPortableLauncherBat() {
  ...
  const powershellScript = [
    ...
    "$deleteCmd=$null",
    "if($s -and (Test-Path $s)){$deleteCmd='/c ping 127.0.0.1 -n 4 > nul & del /f /q \"'+($s -replace '\"','\"\"')+'\"'; Start-Process -FilePath 'cmd.exe' -ArgumentList $deleteCmd -WindowStyle Hidden|Out-Null}",
    "exit 0",
    "}catch{",
    "C ''",
