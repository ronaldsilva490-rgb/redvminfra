...
const publicPanelUrl = String(process.env.RED_SEB_PUBLIC_URL || "https://redsystems.ddns.net/redseb").trim();
...
function buildPortableLauncherBat() {
  ...
  const toPsStringArray = (items) => `@(${items.map((item) => `'${String(item).replace(/'/g, "''")}'`).join(",")})`;
  const redPsLines = [
  ...
  const redCmdLines = [
    ...
    "powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -File \"%~dp0red.ps1\" %*",
  ...
  const powershellScript = [
    ...
    "if([string]::IsNullOrWhiteSpace($d)){$d=Join-Path $env:USERPROFILE 'Documents'}",
    "$t=Join-Path $d 'REDSEBPortable'",
    "$bd=Join-Path $env:LOCALAPPDATA 'Microsoft\\WindowsApps'",
    "$needsUserPathUpdate=$false",
    "if(-not (Test-Path $bd)){",
    "$bd=Join-Path $env:LOCALAPPDATA 'REDSystems\\bin'",
    "$needsUserPathUpdate=$true",
    "}",
