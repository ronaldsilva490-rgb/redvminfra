...
function buildPortableLauncherBat() {
  ...
  const powershellScript = [
    ...
    "function S($n,$m){Write-Host ('['+$n+'] '+$m) -ForegroundColor DarkGray}",
    "try{",
    "if($Host.UI.RawUI){$Host.UI.RawUI.WindowTitle='RED Systems | RED SEB Universal'}",
    "Clear-Host",
    "L",
    "C '                     RED SYSTEMS' 'Red'",
    "C '                 RED SEB UNIVERSAL' 'White'",
    "L",
    "C 'Launcher em modo usuario, sem pedir administrador.' 'DarkGray'",
    "C ('Painel : '+$p) 'DarkGray'",
    "C ('Destino: '+$t) 'DarkGray'",
    "C ''",
    "S 1 'Recebendo o exame'",
    ...
    "}else{",
    "S 2 'RED SEB Portable ja encontrado'",
    "}",
    ...
    "S 4 'Abrindo o RED SEB Portable'",
    ...
    "C ''",
    "C 'RED SEB iniciado com sucesso.' 'Green'",
    "C 'Este launcher sera removido automaticamente.' 'DarkGray'",
    "Start-Sleep -Milliseconds 1200",
    "if($s -and (Test-Path $s)){Start-Process -FilePath 'powershell.exe' -ArgumentList '-NoLogo','-NoProfile','-WindowStyle','Hidden','-Command',\"Start-Sleep -Seconds 2; for($i=0; $i -lt 12; $i++){ try { Remove-Item -LiteralPath '$s' -Force -ErrorAction Stop; break } catch { Start-Sleep -Milliseconds 500 } }\" -WindowStyle Hidden|Out-Null}",
    "exit 0",
    "}catch{",
    "C ''",
